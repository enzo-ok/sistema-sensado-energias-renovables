...
struct str_Config {
	...
	uint16_t rssi_min;
	uint16_t rssi_max;
	uint16_t NonUsed;			//Espacio para igualar el tama�o de las cargas
};
struct str_RespSample {
	...
	int16_t rssi;	//Indicador de fuerza de la se�al recibida en la muestra
	uint32_t Timestamp;	//Este campo se utiliza para saber cuando se tomo la muestra
};

struct str_ReqSample {
	uint16_t NumberOfSample; //Direccion del nodo con el que se tomo la muestra (No es ni Tx ni Rx de la comunicacion)
	uint16_t NonUsed; //Este campo se utiliza para saber cuando se tomo la muestra
	uint32_t NonUsed2; //Indicador de fuerza de la se�al recibida en la muestra
};
