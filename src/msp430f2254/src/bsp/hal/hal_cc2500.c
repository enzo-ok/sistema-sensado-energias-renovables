...
extern const pinConfig cc_gdo0, cc_gdo2, cc_spiSOMI, cc_spiSIMO, cc_spiCLK,
		cc_spinCS;
...
uint8_t hal_cc2500_sendPacket(uint8_t addr, uint8_t *txBuffer, uint8_t size) {
	...
	if (hal_pin_in(&cc_gdo2)) {             //Si esta libre el canal
		...
		if (((status & TI_CCxxx0_STATUS_STATE) == TI_CCxxx0_STATUS_TX)
			...
			for (i = 0; i < TI_CC_TX_TIMEOUT && !hal_pin_in(&cc_gdo0); i++)
				// Wait GDO0 to go hi -> sync TX'ed
				;

			for (j = 0; j < TI_CC_TX_TIMEOUT && hal_pin_in(&cc_gdo0); j++)
				// Wait GDO0 to clear -> end of pkt
				;

			if (i != TI_CC_TX_TIMEOUT && j != TI_CC_TX_TIMEOUT) {
				ok = 1;
	...
	if (rxFifo) {                                         // If it�s not empty
		...
		if (((pktLen + 2) < rxFifo) && (pktLen <= (*length + 1))) { // If size of (pktLen + RSSI + LQI) < RX FIFO and
			...
			*lqi = (status[TI_CCxxx0_LQI_RX] & 0x7F);              // Return LQI
			...
			rxFifo = (hal_cc2500_readStatus(TI_CCxxx0_RXBYTES)
					& TI_CCxxx0_NUM_RXBYTES);     // Read the size of RX FIFO
			...
			return (uint8_t) (status[TI_CCxxx0_LQI_RX] & TI_CCxxx0_CRC_OK); // Return CRC_OK bit
...
int16_t _cc2500_rssiDecToDbm(uint8_t rssiDec){
	...
	if(rssiDec >= 128)
		rssiDbm = (int16_t)((int16_t)(rssiDec - 256) / 2) - TI_CC25000_RSSI_OFFSET;
	else
		rssiDbm = (rssiDec / 2) - TI_CC25000_RSSI_OFFSET;
